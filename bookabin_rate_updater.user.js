...
(function () {
    var WASTE_TYPES = ['General Waste', 'Mixed Heavy Waste', 'Cleanfill/Hardfill', 'Green Garden Waste', 'Soil / Dirt'];
    var BIN_SIZES   = ['2', '3', '4', '4.5', '6', '7.5'];
    var stopFlag = false;
    ...
    async function runUpdates(items) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (!row) { log(wt + ' ' + sz + 'm³: row not found — skipping.', '#f38ba8'); continue; }

            var editBtn = row.querySelector('input[alt="Edit Row"], input[title="Edit Row"]');
            if (!editBtn) { log(wt + ' ' + sz + 'm³: Edit button not found — skipping.', '#f38ba8'); continue; }

            log('Updating ' + wt + ' ' + sz + 'm³ → $' + price);
            ...
            try {
                ...
                log(wt + ' ' + sz + 'm³: edit mode timed out — skipping.', '#f38ba8');
            ...
            if (!priceInput) {
                log(wt + ' ' + sz + 'm³: price input not found — cancelling.', '#f38ba8');
            ...
            if (!updateBtn) { log(wt + ' ' + sz + 'm³: Update Row button missing.', '#f38ba8'); continue; }
    ...
    function buildPanel() {
        ...
        p.id = 'bb-panel';
        p.style.cssText = 'position:fixed;top:20px;right:20px;width:620px;background:#1e1e2e;color:#cdd6f4;border:1px solid #585b70;border-radius:10px;padding:14px 16px;font-family:sans-serif;font-size:13px;z-index:99999;box-shadow:0 6px 28px rgba(0,0,0,.65);';
        ...
        // 2D Table: waste types as rows, bin sizes as columns
        var tbl = document.createElement('table');
        tbl.style.cssText = 'width:100%;border-collapse:collapse;margin-bottom:8px;table-layout:fixed;';

        // Header row
        var thead = tbl.createTHead(), hrow = thead.insertRow();
        var thWt = document.createElement('th'); thWt.textContent = 'Waste Type';
        thWt.style.cssText = 'text-align:left;padding:4px 6px;font-size:11px;color:#a6adc8;border-bottom:1px solid #45475a;width:130px;';
        hrow.appendChild(thWt);
        BIN_SIZES.forEach(function(sz) {
            var th = document.createElement('th'); th.textContent = sz + ' m³';
            th.style.cssText = 'text-align:center;padding:4px 4px;font-size:11px;color:#a6adc8;border-bottom:1px solid #45475a;';
            hrow.appendChild(th);
        });

        // Data rows
        var tbody = tbl.createTBody();
        WASTE_TYPES.forEach(function(wt) {
            var tr = tbody.insertRow();
            var td0 = tr.insertCell();
            td0.textContent = wt;
            td0.style.cssText = 'padding:4px 6px;font-size:11px;border-bottom:1px solid #313244;color:#a6e3a1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;';
            BIN_SIZES.forEach(function(sz) {
                var td = tr.insertCell();
                td.style.cssText = 'padding:3px 4px;border-bottom:1px solid #313244;';
                var inp = document.createElement('input');
                inp.type = 'number'; inp.min = '0'; inp.placeholder = '-';
                inp.id = 'bb-p-' + wt.replace(/[^a-z0-9]/gi, '_') + '-' + sz;
                inp.style.cssText = 'width:100%;box-sizing:border-box;background:#181825;color:#cdd6f4;border:1px solid #585b70;border-radius:4px;padding:3px 4px;font-size:11px;text-align:center;';
                td.appendChild(inp);
            });
        });
        ...
        clr.addEventListener('click', function() {
            WASTE_TYPES.forEach(function(wt) {
                BIN_SIZES.forEach(function(sz) {
                    var e = document.getElementById('bb-p-' + wt.replace(/[^a-z0-9]/gi, '_') + '-' + sz);
                    if (e) e.value = '';
                });
            });
        ...
        run.addEventListener('click', function() {
            ...
            WASTE_TYPES.forEach(function(wt) {
                BIN_SIZES.forEach(function(sz) {
                    ...
                    if (!isNaN(pr)) items.push({ wasteType: wt, size: sz, price: pr });
                });
            });
